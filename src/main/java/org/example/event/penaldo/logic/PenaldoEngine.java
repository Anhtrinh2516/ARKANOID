package org.example.event.penaldo.logic;
...
public class PenaldoEngine extends EventGameEngine {
    ...
    protected void onLevelComplete(int completedLevel) {
        if (completedLevel == 1) {
            org.example.controller.SkinManager.INSTANCE.unlockPaddleSkin(
                org.example.controller.SkinManager.PaddleSkin.PENALDO
            );
            org.example.controller.SkinManager.INSTANCE.unlockBallSkin(
                org.example.controller.SkinManager.BallSkin.PENALDO
            );
        }
    }
    ...
    protected void showWinScreen() {
        javafx.application.Platform.runLater(() -> {
            try {
                boolean play = org.example.ui.TaiXiuDialog.showAskDialog();
                
                if (play) {
                    int currentScore = getScore();
                    int currentMoney = getMoney();
                    org.example.ui.TaiXiuDialog dialog = new org.example.ui.TaiXiuDialog(currentScore, currentMoney);
